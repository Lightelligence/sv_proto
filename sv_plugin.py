    ...
    def sv_deserialize(self, indent, result_var="", field_number_prefix=""):
        ...
        if self.sv_map:
            ...
            if self.type == self.TYPE_MESSAGE:
                ...
                lines.append(f"    `uvm_fatal(this.get_name(), \"Unexpected wire_type\")")
                lines.append(f"  end")
                ...
                lines.append(f"    `uvm_fatal(this.get_name(), \"Unexpected wire_type\")")
                lines.append(f"  end")
                lines.append(f"  if ({PB_PKG}::{self.sv_decode_func}(._result({result_var}), ._stream(_stream), ._cursor(_cursor), ._str_length(delimited_length))) begin")
                lines.append(f"    `uvm_fatal(this.get_name(), \"Failed subdecode\")")
                lines.append(f"  end")
                if self.sv_queue:
                    lines.append(f"  this.{self.name}.push_back({result_var});")
            else:
                lines.append(f"  if (!((wire_type == {PB_PKG}::WIRE_TYPE_DELIMITED) || (wire_type == {PB_PKG}::{self.sv_wire_type}))) begin")
                lines.append(f"    `uvm_fatal(this.get_name(), \"Unexpected wire_type\")")
                lines.append(f"  end")
                ...
                lines.append(f"    if ({PB_PKG}::{self.sv_decode_func}(._result({result_var}), ._stream(_stream), ._cursor(_cursor))) begin")
                lines.append(f"    `uvm_fatal(this.get_name(), \"Failed subdecode\")")
                lines.append(f"  end")
                if self.type == self.TYPE_ENUM:
...
class SVMapDescriptorProto(SVDescriptorProto):
    ...
    def sv_deserialize(self, indent, varname, field_number_prefix):
        ...
        lines.append(f"    if (pb_pkg::decode_message_key(._field_number(field_number),")
        lines.append(f"                                        ._wire_type(wire_type),")
        lines.append(f"                                        ._stream(_stream),")
        lines.append(f"                                        ._cursor(_cursor))) begin")
        lines.append(f"    `uvm_fatal(this.get_name(), \"failed decode_message_key\")")
        lines.append(f"    end")
        lines.append(f"    if (wire_type == {PB_PKG}::WIRE_TYPE_DELIMITED) begin")
        lines.append(f"        if ({PB_PKG}::decode_varint(._value(delimited_length),")
        lines.append("                                       ._stream(_stream),")
        lines.append("                                       ._cursor(_cursor))) begin")
        lines.append("           `uvm_fatal(this.get_name(), \"failed to decode delimited_length\")")
        lines.append("         end")
        lines.append("    end")
        ...
        lines.append(f"        if (pb_pkg::decode_and_consume_unknown(._wire_type(wire_type), ._stream(_stream), ._cursor(_cursor), ._delimited_length(delimited_length))) begin")
        lines.append(f"          `uvm_fatal(this.get_name(), \"Failed decode_and_consume_unknown\")")
        lines.append(f"        end")
        lines.append(f"      end")
    ...
    for proto_file in request.proto_file:
        ...
        for item, package in traverse(proto_file):
            ...
            if isinstance(item, DescriptorProto):
                ...
                pkg.append(f"      if (!this.is_initialized()) begin")
                pkg.append(f"        `uvm_fatal(this.get_name(), \"Attempting to serialize, but not initialized\")")
                pkg.append(f"      end")
                for f in item.sv_fields:
                ...
                pkg.append(f"      {PB_PKG}::cursor_t cursor_orig = _cursor;")
                ...
                pkg.append(f"        if ({PB_PKG}::decode_message_key(._field_number(field_number),")
                pkg.append("                                          ._wire_type(wire_type),")
                pkg.append("                                          ._stream(_stream),")
                pkg.append("                                          ._cursor(_cursor))) begin")
                pkg.append("            `uvm_fatal(this.get_name(), \"Decode_message_key failed\")")
                pkg.append("         end")
                pkg.append(f"        if (wire_type == {PB_PKG}::WIRE_TYPE_DELIMITED) begin")
                pkg.append(f"            if ({PB_PKG}::decode_varint(._value(delimited_length),")
                pkg.append("                                         ._stream(_stream),")
                pkg.append("                                         ._cursor(_cursor))) begin")
                pkg.append("               `uvm_fatal(this.get_name(), \"decode_varint for WIRE_TYPE_DELIMITED failed\")")
                pkg.append("             end")
                ...
                pkg.append("          0 : `uvm_error(this.get_name(), $sformatf(\"Extracted field_number==0 which is illegal at cursor %0d in stream\\n%p\", _cursor, _stream))")
                for f in item.sv_fields:
                    pkg.extend(f.sv_deserialize(indent=10))
                pkg.append(f"          default : begin")
                pkg.append(f"            if ({PB_PKG}::decode_and_consume_unknown(._wire_type(wire_type), ._stream(_stream), ._cursor(_cursor), ._delimited_length(delimited_length))) begin")
                pkg.append(f"              `uvm_fatal(this.get_name(), \"Failed decode_and_consume_unknown\")")
                pkg.append(f"            end")
                pkg.append(f"          end")
                pkg.append("        endcase")
                pkg.append(f"        if (wire_type == {PB_PKG}::WIRE_TYPE_DELIMITED) begin")
                pkg.append("          if (_cursor != delimited_stop) begin")
                pkg.append("            `uvm_fatal(this.get_name(), \"cursor didn't stop at exactly delimited_stop\")")
                pkg.append("          end")
                pkg.append("        end")
                pkg.append("        if (_cursor == cursor_orig) begin")
                pkg.append("          `uvm_error(this.get_name(), $sformatf(\"Deserialize loop didn't advance cursor starting at position %0d. Stream follows:\\n%p\", _cursor, _stream))")
                pkg.append("          break;")
                pkg.append("        end")
                pkg.append("      end")
